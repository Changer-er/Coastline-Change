...
def smoothn(y,nS0=10,axis=None,smoothOrder=2.0,sd=None,verbose=False,\
  ...
  # (Inf/NaN values = missing derived_data).
  ...
  if any(W<0):
    raise RuntimeError('smoothn:NegativeWeights',\
      ...
      pass
  #---
  # Weighted or missing derived_data?
  isweighted = any(W != 1);
  #---
  # Robust smoothing?
  #isrobust
  ...
  if isweighted:
    #--- With weighted/missing derived_data
  ...
  #---
  z0 = z;
  y[~IsFinite] = 0; # arbitrary values for missing y-derived_data
  #---
  tol = 1.;
  ...
  while RobustIterativeProcess:
    ...
    while tol>TolZ and nit<MaxIter:
        ...
        # if no weighted/missing derived_data => tol=0 (no iteration)
...
def gcv(p,Lambda,aow,DCTy,IsFinite,Wtot,y,nof,noe,smoothOrder):
    ...
    if aow>0.9: # aow = 1 means that all of the derived_data are equally weighted
...
## Initial Guess with weighted/missing derived_data
...
def test3(axis=None):
   ...
   y[I[1:n**2*0.5]] = nan; # lose 50% of derived_data
   y = y.reshape(y0.shape)
   y[40:90,140:190] = nan; # create a hole
   yData = y.copy()
   z0,s,exitflag,Wtot = smoothn(yData,axis=axis); # smooth derived_data
   yData = y.copy()
   z,s,exitflag,Wtot = smoothn(yData,isrobust=True,axis=axis); # smooth derived_data
   y = yData
